...
function NavbarCustom({ placeholder, setPlaceholder }) {
	const [navbarExpanded, setNavbarExpanded] = useState(false);

	const responsiveNavbarNav = useRef();
	const navbarToggler = useRef();
	const highlightsIcons = useRef();
	const navbarCollapseBackdrop = useRef();

	const hideResponsiveNavbarNav = () => {
		/* responsiveNavbarNav.current.classList.add('collapsing');
		responsiveNavbarNav.current.classList.remove('show');
		navbarToggler.current.classList.add('collapsed'); */
		/* highlightsIcons.current.classList.remove('d-none'); */
	};
	const showResponsiveNavbarNav = () => {
		/* navbarToggler.current.classList.remove('collapsed');
		responsiveNavbarNav.current.classList.add('collapsing');
		responsiveNavbarNav.current.classList.add('show'); */
		/* highlightsIcons.current.classList.add('d-none'); */
	};
	return (
		<Navbar
			id="navbar-custom"
		...
		C920.258,569.838,845.563,478.728,845.563,478.728z"
			...
			<Navbar.Toggle
				ref={navbarToggler}
				/* onClick={showResponsiveNavbarNav} */
				onClick={(e) => {
					e.stopPropagation();
			...
			/>
			<Navbar.Collapse ref={responsiveNavbarNav} id="responsive-navbar-nav">
				<div
					className="navbar-collapse-backdrop"
					ref={navbarCollapseBackdrop}
				></div>
				<Nav className="mr-auto">
					<NavLink
						onClick={hideResponsiveNavbarNav}
						className="nav-link"
						role="button"
						to="/exercises"
					>
			...
			<HighlightsIcons ref={highlightsIcons} {...{ navbarExpanded }} />
		</Navbar>
	);
}
